...
def publish_post(post_data):
    ...
    try:
        ...
        print(f"✅ Publicado: {post_data['day']} - {post_data['category']}")
        return True
    except Exception as e:
        print(f"❌ Error publicando {post_data['day']}: {e}")
        return False


def get_today_post(calendar):
    """Obtiene la publicación correspondiente al día de hoy."""
...
def main():
    ...
    if args.all:
        print("🚀 Publicando TODO el calendario...\n")
        success = 0
        for post in calendar:
        ...
        print(f"🚀 Publicando publicación de {args.day}...\n")
        post = next((p for p in calendar if p["day"] == args.day), None)
        if post:
            publish_post(post)
        else:
            print(f"❌ No se encontró publicación para {args.day}")

    else:
        print("🚀 Publicando publicación del día de hoy...\n")
        post = get_today_post(calendar)
        if post:
            publish_post(post)
        else:
            print("❌ No se encontró publicación para hoy")
